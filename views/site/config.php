...
<div class="trailer-load-create">
  ...
  <?php if ( $items ) foreach ( $items as $item ) { ?>
    <?php $form = ActiveForm::begin( [
      ...
      'fieldConfig' => [
        'template'     => "{label}\n<div class=\"col-lg-9\">{input}</div><br><br><div class=\"col-lg-9 col-lg-offset-3\">{error}</div><div class=\"col-lg-9 col-lg-offset-3\">{hint}</div>",
      ...
      <div class="col-lg-8">
        <?php
          switch ( $item->type ) {
            ...
            case 'size':
              echo $form->field( $item, 'value' )->textInput( [ 'class' => 'number form-control', 'min' => 1 ] )->hint( $item->description );
              break;
          }
        ?>
      </div>
